 ...
 * Post-Condition: Instantiate spinners and loading of JSON
 */
function init() {

    setTimeout(function() {
        // Load
        d3.json(BASEURL + "/metrics/", function(response) {
            var chartsToMake = countUp(response);

            if (!chartsToMake) {
                return;
            }

            $(".loadingInfo").fadeOut(500);
            $(".spinnerLoader").hide();

            makeTypeChart(chartsToMake.types, "Indicators By Type", "perTypeChart");
            makeTypeChart(chartsToMake.sources, "Indicators By Source", "perSourceChart");
        });
 ...
 * Pre-Condition:  Document loaded
 * Post-Condition: Count of IP, Domain, and URL returned
*/
function countUp(response) {

  var perDayRaw = response.perday;
  var perSourceRaw = response.persource;
  var perTypeRaw = response.pertype;

  if (!perDayRaw || !perSourceRaw || !perTypeRaw) {
     $(".loadingInfo").fadeOut(500);
     $(".spinnerLoader").hide();
     $('#emptyNotice').slideDown(500);
     $('#emptyNotice').append('<strong>Notice</strong> No indicators found! Configure the Twitter parser or set up feeds in the <a href="settings.php">Settings</a> to allow GOSINT to begin scraping for indicators from the Internet.');
     return;
  }

  // perType data prep
  // Build types data object
  var typesData = [];
  for (var i = 0; i < perTypeRaw.length; i++) {
      newType = new Object();
      newType.label = perTypeRaw[i].type;
      newType.value = perTypeRaw[i].number;
  ...
  var sourcesData = [];
  for (var i = 0; i < perSourceRaw.length; i++) {
      var extracted = extractRootDomain(perSourceRaw[i].source);
      var index = checkSourceInArray(extracted, sourcesData);
      if (index == -1) {
          var newSource = new Object();
          newSource.label = extracted;
          newSource.value = perSourceRaw[i].number;
          sourcesData.push(newSource);
      }
      else {
          sourcesData[index].value += perSourceRaw[i].number;
      }

  }

  var returnvalue = {"types": typesData, "sources": sourcesData};
  return returnvalue;

}

/*
 * Function Name:  makeTypeChart
 * Pre-Condition:  mydata contains valid JSON object, mychartid is valid div
 * Post-Condition: Return d3pie chart with provided data and configured settings
 */
function makeTypeChart(mydata, mytitle, mychartid) {
    var pie = new d3pie(mychartid, {
      "header": {
      	"title": {
      		"text": mytitle,
      		"fontSize": 24,
